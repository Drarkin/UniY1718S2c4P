...
char HiddenMsgBuffer[RingMsgSize_MAX];
char *RingMsgBuffer=HiddenMsgBuffer;//default buffer array
char TOKEN;
...
char* RingReadMSG(){
	...
	msgSize=RingReadMsgFrom(RingInfo.B_fd);
	if (msgSize>0){
		fprintf(stderr,"%s\n",RingMsgBuffer);
...
int RingReadMsgFrom(int fd){
	//read msg from fd
	//return bytes read
	for (msgSize=0;msgSize<RingMsgSize_MAX;msgSize++){ RingMsgBuffer[msgSize]='\0';}
	return (int) read(fd,RingMsgBuffer,RingMsgSize_MAX-1);
}
...
int OuroborosTail(int B_fd,struct sockaddr_in *B_addr,int myId){
	...
	char *msgBuffer=RingMsgBuffer;//worst msg size plus 1
	char ip[MYIPSIZE];
